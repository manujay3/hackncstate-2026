  ...
  return (
    <SectionContainer>
      <button
        ...
        className={`flex items-center gap-2 text-sm text-[#71717a] hover:text-[#e4e4e7] transition-colors ${hasData ? 'cursor-pointer' : 'cursor-default'}`}
      >
        <svg
        ...
        <span className="font-medium uppercase tracking-widest text-xs">Technical Metadata</span>
      </button>
      {open && (
          ...
          {hasData ? (
            metadata.map((m) => (
              <div key={m.label} className="flex items-baseline gap-2">
                <span className="text-sm text-[#71717a] shrink-0">{m.label}</span>
                <span className="text-sm text-[#e4e4e7] font-mono">{m.value}</span>
              </div>
            ))
          ) : (
